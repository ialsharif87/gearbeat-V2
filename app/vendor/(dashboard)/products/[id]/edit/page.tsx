...
export default async function VendorProductEditPage({
  ...
  async function updateProduct(formData: FormData) {
    ...
    if (nextError) {
      redirect(
        `/vendor/products/${productId}/edit?error=${encodeURIComponent(nextError)}`
      );
    }
    ...
    redirect(`/vendor/products/${productId}/edit?saved=1`);
  }
  ...
  return (
    <main className="dashboard-page" style={{ maxWidth: 1120, margin: "0 auto" }}>
      ...
      <form action={updateProduct} className="card" style={{ marginTop: 24, display: "grid", gap: 18 }}>
        ...
        >
          <h2>
            <T en="Simple variant" ar="خيار المنتج" />
          </h2>

          <p style={{ color: "var(--muted)", lineHeight: 1.7 }}>
            <T
              en="Optional. Use this for one product variant such as Standard, Black, or Bundle."
              ar="اختياري. استخدمه لخيار واحد مثل Standard أو Black أو Bundle."
            />
          </p>
          ...
          <div className="grid grid-4" style={{ marginTop: 14 }}>
            <div>
              <label>
                <T en="Variant name" ar="اسم الخيار" />
              </label>
              <input
                name="variant_name"
                className="input"
            ...
            <div>
              <label>
                <T en="Variant SKU" ar="SKU للخيار" />
              </label>
              <input
                name="variant_sku"
                className="input"
                defaultValue={firstVariant?.sku || ""}
                placeholder={`${product.sku || "SKU"}-STD`}
              />
            </div>

            <div>
              <label>
                <T en="Price adjustment" ar="فرق السعر" />
              </label>
              <input
            ...
            <div>
              <label>
                <T en="Variant stock" ar="مخزون الخيار" />
              </label>
              <input
                ...
                type="number"
                min="0"
                defaultValue={firstVariant?.stock_quantity || ""}
              />
            </div>
          </div>
        </section>
        ...
        <button
          ...
          disabled={categoryRows.length === 0 || brandRows.length === 0}
        >

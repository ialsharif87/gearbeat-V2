...
export default function JoinStudioPage() {
  ...
  async function handleSubmit(formData: FormData) {
    startTransition(async () => {
      const res = await submitProviderLead(formData, "studio");
      setResult(res);
    });
  }
      ...
      <main className="dashboard-page" style={{ maxWidth: 600, margin: "100px auto", textAlign: "center" }}>
        <div className="card" style={{ padding: "60px 40px" }}>
          ...
          <p style={{ color: "var(--muted)", fontSize: "1.1rem", lineHeight: 1.6 }}>
            <T 
              en="Thank you for your interest. Our team will review your details and reach out to you within 2 business days." 
              ar="شكراً لاهتمامك. سيقوم فريقنا بمراجعة بياناتك والتواصل معك خلال يومي عمل." 
            />
          </p>
        </div>
      </main>
    ...
    <main className="dashboard-page" style={{ maxWidth: 600, margin: "60px auto" }}>
      <div style={{ textAlign: "center", marginBottom: 40 }}>
        <h1 style={{ fontSize: "2.2rem", marginBottom: 16 }}>
          <T en="List your studio on GearBeat" ar="سجّل استوديوك في GearBeat" />
        </h1>
      ...
      <div className="card" style={{ padding: 40 }}>
        <form action={handleSubmit} style={{ display: "grid", gap: 24 }}>
          <div style={{ display: "grid", gap: 8 }}>
            ...
            <input name="name" type="text" className="input" required placeholder="e.g. Abdullah Ahmed" />
          </div>

          <div style={{ display: "grid", gap: 8 }}>
            <label style={{ fontSize: "0.9rem", fontWeight: 600 }}>
              <T en="Business Name" ar="اسم المنشأة / الاستوديو" />
            </label>
            <input name="business_name" type="text" className="input" required placeholder="e.g. Echo Studios" />
          </div>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            ...
            <input name="email" type="email" className="input" required placeholder="name@company.com" />
          </div>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            ...
            <textarea name="message" className="input" style={{ minHeight: 120, paddingTop: 12 }} placeholder="Tell us more about your studio..." />
          </div>

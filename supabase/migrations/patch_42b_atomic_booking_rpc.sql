-- Patch 42B1: Atomic Booking RPC Foundation
-- This RPC ensures that bookings are created atomically, preventing double-bookings.

-- 1. Update bookings status check to ensure 'pending_payment' is allowed
DO $$ 
DECLARE
...
-- 2. Create the Atomic Booking RPC
...
BEGIN
  ...
  -- 2. Check for overlapping bookings in active statuses
  -- We include pending_payment, pending, accepted, confirmed, and paid (if applicable)
  SELECT id INTO v_conflict_id
  FROM public.bookings
  WHERE studio_id = p_studio_id
    AND booking_date = p_booking_date
    AND status IN ('pending_payment', 'pending', 'confirmed', 'paid', 'accepted')
...
-- 4. Restrict execution to service role
REVOKE EXECUTE ON FUNCTION public.create_studio_booking_v1(UUID, UUID, DATE, TIME, TIME, NUMERIC, NUMERIC, NUMERIC, TEXT, TEXT, TEXT, UUID, TEXT, JSONB, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_studio_booking_v1(UUID, UUID, DATE, TIME, TIME, NUMERIC, NUMERIC, NUMERIC, TEXT, TEXT, TEXT, UUID, TEXT, JSONB, TEXT) TO service_role;
